...
当访问一个实例上的一个属性时，如果在实例身上没有这个属性就会去她的隐式原型`__proto__`上去找，一直找下去，向父对象在向爷爷对象，如果最后返回null则没有这个值，这样，通过`__proto__`属性将对象连接起来的这条链路就叫原型链
...
图片懒加载就是当用户浏览到某个页面的时候再进行记载这个图片，一般的做法就是先把这个图片的src属性写成空或者一个loadding图片，并为这个图片设置一个自定义属性data-src为图片的真实路径，通过函数判断如果浏览到某个区域，则将img.src=img.getAttribute('data-src')即可
...
## 4. Hooks 与 Class 中调用 setState 有不同的表现差异么?
